...
class Parse extends Tester\TestCase
{
	...
	{
		$query = Db\Helper::prepareSql($this->connection->prepareQuery('SELECT * FROM table'));
	...
	{
		$query = Db\Helper::prepareSql($this->connection->prepareQuery('SELECT * FROM table WHERE column = $1', 1));
		Tester\Assert::same('SELECT * FROM table WHERE column = $1', $query->getSql());
		Tester\Assert::same([1], $query->getParams());

		$query = Db\Helper::prepareSql($this->connection->prepareQueryArray('SELECT * FROM table WHERE column = $1', [1]));
		Tester\Assert::same('SELECT * FROM table WHERE column = $1', $query->getSql());
		Tester\Assert::same([1], $query->getParams());

		$query = Db\Helper::prepareSql($this->connection->prepareQuery('SELECT * FROM table WHERE column = ?', 1));
		Tester\Assert::same('SELECT * FROM table WHERE column = $1', $query->getSql());
		Tester\Assert::same([1], $query->getParams());

		$query = Db\Helper::prepareSql($this->connection->prepareQueryArray('SELECT * FROM table WHERE column = ?', [1]));
		Tester\Assert::same('SELECT * FROM table WHERE column = $1', $query->getSql());
		Tester\Assert::same([1], $query->getParams());
	}
	...
	public function testPrepareQueryWithBadParams()
	{
		Tester\Assert::exception(function() {
			$query = Db\Helper::prepareSql($this->connection->prepareQuery('SELECT * FROM table WHERE column = ? AND column2 = ?', 1));
			$query->getSql();
	...
	{
		$query = Db\Helper::prepareSql($this->connection->prepareQuery('SELECT * FROM ? WHERE column = ?', $this->connection::literal('table'), 1));
		Tester\Assert::same('SELECT * FROM table WHERE column = $1', $query->getSql());
		Tester\Assert::same([1], $query->getParams());
	}
	...
	{
		$query = Db\Helper::prepareSql($this->connection->prepareQuery('SELECT * FROM table WHERE column IN (?)', [1, 2]));
	...
	public function testPrepareQueryWithQuery()
	{
		...
		$query = Db\Helper::prepareSql($this->connection->prepareQuery('SELECT * FROM table WHERE id IN (?)', $subquery));
	...
	{
		$query = Db\Helper::prepareSql($this->connection->prepareQuery('SELECT * FROM table WHERE column = ? AND text ILIKE \'What\?\'', 1));
	...
	public function testPrepareQueryComplex()
	{
		...
		$query = Db\Helper::prepareSql($this->connection->prepareQuery(
